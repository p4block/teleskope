...
export function ResourceDetail({
  ...
  // Fetch data only when we have a resource name
  ...
  const handleExec = (containerName?: string) => {
    if (resource.kind === "Pod" && resourceName) {
      execPod.mutate({
  ...
  const handleDelete = () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${resource.kind} ${resourceName}?`,
      )
    ) {
      deleteResource.mutate(
        ...
        },
        {
          ...
          onError: (error) => {
            alert(
              `Failed to delete ${resource.kind} ${resourceName}: ${error.message}`,
            );
          },
        },
      );
  ...
  const detailSections = useMemo(() => {
    if (!data) return [];
    return getDetailSections(resource.kind, data, {
      onExec: handleExec,
      onLogs: handleLogs,
    });
      ...
      <header className="detail-header">
        <div className="detail-header-content">
          ...
          <h2 className="detail-title">
            {!resourceName ? "Select a resource" : resourceName}
          </h2>
          {namespace && (
                ...
                <div className="detail-section-content">
                  {related.map((res: any, i: number) => (
                    <div key={i} className="detail-field">
                        ...
                        onClick={() => {
                          if (onNavigate) {
                            ...
                            const info: ApiResourceInfo = {
                              group:
                                res.apiVersion.split("/")[0] === res.apiVersion
                                  ? ""
                                  : res.apiVersion.split("/")[0],
                              version:
                                res.apiVersion.split("/")[1] || res.apiVersion,
                              kind: resKind,
                      ...
                      >
                        {res.metadata.name}{" "}
                        <span style={{ opacity: 0.5 }}>({res.kind})</span>
                      </span>
                    </div>
                  ))}
                </div>
...
function getDetailSections(
  ...
  const metadataSection: SectionData = {
    ...
    items: [
      ...
      {
        label: "Created",
        value: formatAge(String(metadata?.creationTimestamp)) + " ago",
      },
      {
        label: "Resource Version",
        value: String(metadata?.resourceVersion || "-"),
      },
    ],
  };
  ...
  switch (kind) {
    case "Pod":
      return [
        metadataSection,
        ...getPodSections(spec, status, actions.onExec, actions.onLogs),
      ];
    case "Deployment":
  ...
  return [
    ...
    },
    {
      ...
      items: containers.map((c, i) => {
        const containerStatus = containerStatuses[i] as
          | Record<string, unknown>
          | undefined;
        ...
        return {
          ...
          value: (
            ...
            >
              ...
              >{`${String(c.image).split("/").pop()} | Ready: ${ready} | Restarts: ${restarts}`}</span>
              <div style={{ display: "flex", gap: "0.25rem" }}>
                <button
                  className="btn btn-secondary btn-sm"
                  style={{ fontSize: "0.625rem", height: "1.25rem" }}
  ...
  return [
    {
      ...
      items: [
        ...
        { label: "Available", value: String(status?.availableReplicas || 0) },
        {
          label: "Strategy",
          value: String(
            (spec?.strategy as Record<string, unknown>)?.type || "-",
          ),
        },
      ],
    },
  ...
  return [
    {
      ...
      items: [
        ...
        { label: "Cluster IP", value: String(spec?.clusterIP || "-") },
        {
          ...
          value: String(spec?.sessionAffinity || "None"),
        },
      ],
    },
    {
...
function getIngressSections(
  ...
  const ingress = ((status?.loadBalancer as Record<string, unknown>)?.ingress ||
  ...
  return [
    {
      ...
      items: [
        ...
        {
          label: "Address",
          value: ingress.map((i) => i.ip || i.hostname).join(", ") || "-",
        },
      ],
    },
    {
  ...
  return [
    {
      ...
      items: keys.map((key) => ({
        label: key,
        value: String(
          dataMap[key].length > 50
            ? dataMap[key].slice(0, 50) + "..."
            : dataMap[key],
        ),
      })),
    },
...
function getSecretSections(data: Record<string, unknown>): SectionData[] {
  ...
  return [
    {
      title: "Secret Info",
      items: [{ label: "Type", value: String(data.type || "Opaque") }],
    },
    {
      title: "Data Keys",
      items: keys.map((key) => ({
