      ...
      .map((col: ColumnDefinition) => ({
        ...
        cell: ({ getValue }) => {
          ...
          if (col.type === "status") {
            ...
            return (
              <span className={`status-badge ${statusClass}`}>{formatted}</span>
            );
          }
            ...
            return (
              <div className="container-statuses">
                {containerStatuses.map(
                  (container: Record<string, unknown>, index: number) => {
          ...
          <table className="resource-table">
            ...
            <tbody>
              {table.getRowModel().rows.map((row) => {
                const rowStatusClass = getStatusClass(
                  getPodStatus(row.original),
                );
                return (
                  <tr
                    key={row.id}
                    ...
                    style={
                      rowStatusClass === "terminating" ? { opacity: 0.5 } : {}
                    }
