...
interface SidebarProps {
  ...
  currentView: 'resources' | 'dashboard';
}
...
export function Sidebar({
  ...
  const {
    ...
    isLoading,
    error,
  } = useApiResources(!!currentContext);
  ...
  const prevExpandedRef = useRef<Set<string>>(
    new Set(["Workloads", "Network"]),
  );
    ...
    <aside className="sidebar">
      ...
      <div className="px-4 py-2">
        <button
          ...
          className={`w-full text-left px-3 py-2 rounded-md transition-colors flex items-center gap-2 ${currentView === 'dashboard'
      ...
      <nav className="sidebar-content">
        {!currentContext ? (
          ...
          <div
            ...
            style={{ padding: "1rem", color: "var(--color-status-failed)" }}
          >
          ...
          sortedCategories.map((category) => {
            ...
            const isExpanded = search ? true : expandedCategories.has(category);
